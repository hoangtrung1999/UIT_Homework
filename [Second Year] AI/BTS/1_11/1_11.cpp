...
int SelectMachine (vector<int> &workTime, vector<int> &position)
{
...
int main()
{
	...
	// Trường hợp đặc biệt, sẽ làm tổng quát sau
	for (int i = 0; i < m; i++)
		position[i] = i*n;

	for (int i = 0; i < n; i++)
	{
		int machine = SelectMachine(workTime, position) / n;
		saveWork[i] = machine;
		for (int j = 0; j < m; j++)
				position[j] += 1;
	}	
